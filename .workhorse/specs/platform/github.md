...
Without one it stores nothing an attacker wants; with one it stores a credential that can read repositories, on every device the user syncs to.
That is the trade a read-only, checks-only token is chosen to keep small.
