...
Anchors are needed for the composer, the pull request detail toggle and its expanded state, the branch dropdown, the checks row, the review row, the base row, the open workspace switcher menu with its rows, the sidebar's wordmark and the brand mark in both the header and the retracted rail, and the toggle above an open artefact together with the region that artefact renders into.
