...
How Workhorse Pro gets from a green build to an installed add-on that updates itself. Not started — this is the plan for when iteration settles.

## The shape

**Unlisted, self-distributed, signed by AMO.** Signing is not optional: release and beta Firefox refuse unsigned extensions, so the package goes through addons.mozilla.org either way. What listing would add is a public AMO page, full human review, screenshots and a privacy policy — ceremony for an audience of one, and `optional_host_permissions: ["*://*/*"]` plus a stored GitHub token would draw a reviewer's attention for no benefit. Unlisted uploads go through automated review and come back signed.
...
- [ ] `npm ci`, then `tsc --noEmit`, `npm test`, `npm run build`, `web-ext lint --source-dir dist/firefox`
- [ ] Fail on lint errors; the Android `strict_min_version` warning is expected and not an error

All four already pass locally, so this is wiring rather than work.

## 2. Auto-update wiring

- [ ] Add `browser_specific_settings.gecko.update_url` to the Firefox manifest, pointing at the Pages-hosted `updates.json`
- [ ] Generate `updates.json` from the release: the extension id, the new version, and an HTTPS `update_link` to the release asset
- [ ] Publish it to Pages as part of the release job
...
- [ ] release-please action maintaining the release pull request from conventional commits
- [ ] On release creation: `npm ci`, `npm run build`
- [ ] `web-ext sign --source-dir dist/firefox --channel unlisted --api-key … --api-secret …`, which defaults to the AMO v5 submission API
- [ ] Attach the signed `.xpi` from `web-ext-artifacts/` to the GitHub Release
- [ ] Regenerate and publish `updates.json`
